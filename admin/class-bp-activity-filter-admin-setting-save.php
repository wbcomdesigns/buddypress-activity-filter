...
if ( ! class_exists( 'WbCom_BP_Activity_Filter_Admin_Setting_Save' ) ) {
	...
	class WbCom_BP_Activity_Filter_Admin_Setting_Save {
		 ...
		 * Saving options
		 */
		public function bp_core_activity_filter_admin_settings_save(): void {

			// Check if we are on the activity filter settings page and the form is being submitted.
			if ( isset( $_GET['page'] ) && 'bp_activity_filter_settings' === $_GET['page'] && ! empty( $_POST['submit'] ) ) {
				...
				// Save hidden filters if they exist.
				...
				// Save the default profile filter name.
