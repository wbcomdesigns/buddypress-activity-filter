<?php
/**
 * Settings tab: Default Filters (site-wide + profile).
 *
 * Owns options bp_activity_filter_default + bp_activity_filter_profile_default.
 * Ports the legacy "Default Filters" tab 1:1 into the card-panel shell.
 *
 * @package BuddyPress_Activity_Filter
 * @since   3.2.1
 */

defined( 'ABSPATH' ) || exit;

$bpaf_default_filter         = BP_Activity_Filter_Migration::get_option_with_fallback( 'bp_activity_filter_default', '0' );
$bpaf_profile_default_filter = BP_Activity_Filter_Migration::get_option_with_fallback( 'bp_activity_filter_profile_default', '-1' );
$bpaf_activity_actions       = BP_Activity_Filter_Helper::get_activity_actions();

// A hidden type is stripped from the stream by the Hidden Activities tab, so
// choosing it here would leave every visitor looking at an empty stream with no
// explanation. Keep hidden types out of both lists.
//
// One exception: if a hidden type is ALREADY the stored default - either it was
// set before it was hidden, or the site is upgrading - it stays in the list,
// flagged. Dropping it silently would make the select fall back to its first
// option and rewrite the setting on the next save, which is a worse surprise
// than showing the owner the conflict and letting them resolve it.
$bpaf_hidden_activities = get_option( 'bp_activity_filter_hidden', array() );
if ( ! is_array( $bpaf_hidden_activities ) ) {
	$bpaf_hidden_activities = array();
}

/**
 * Should this activity type be offered as a default filter?
 *
 * @param string $key     Activity type key.
 * @param string $current Currently stored value for the select being built.
 * @return bool
 */
$bpaf_offer_as_default = static function ( $key, $current ) use ( $bpaf_hidden_activities ) {
	return ! in_array( $key, $bpaf_hidden_activities, true ) || $key === $current;
};

$bpaf_default_is_hidden         = in_array( $bpaf_default_filter, $bpaf_hidden_activities, true );
$bpaf_profile_default_is_hidden = in_array( $bpaf_profile_default_filter, $bpaf_hidden_activities, true );

// Sentinel: tells the sanitizer which option keys this tab owns so saving
// here never wipes the Hidden options. See Playbook 7.1 and
// BP_Activity_Filter_Admin_Panel::tab_rendered_option().
$bpaf_rendered_options = array(
	'bp_activity_filter_default',
	'bp_activity_filter_profile_default',
);
foreach ( $bpaf_rendered_options as $bpaf_rendered_option ) :
	?>
	<input type="hidden" name="bp_activity_filter_rendered_options[]" value="<?php echo esc_attr( $bpaf_rendered_option ); ?>">
<?php endforeach; ?>

<div class="bpaf-card">
	<div class="bpaf-card__head">
		<p class="bpaf-card__title"><?php esc_html_e( 'Default Activity Filters', 'bp-activity-filter' ); ?></p>
		<p class="bpaf-card__desc"><?php esc_html_e( 'Choose what each activity stream shows by default. Members can still switch the filter themselves using the activity dropdown.', 'bp-activity-filter' ); ?></p>
	</div>
	<table class="form-table" role="presentation">
		<tbody>
			<tr>
				<th scope="row">
					<label for="bp_activity_filter_default"><?php esc_html_e( 'Site-wide stream default', 'bp-activity-filter' ); ?></label>
				</th>
				<td>
					<select name="bp_activity_filter_default" id="bp_activity_filter_default" class="regular-text">
						<option value="0" <?php selected( $bpaf_default_filter, '0' ); ?>>
							<?php esc_html_e( 'Everything', 'bp-activity-filter' ); ?>
						</option>
						<?php foreach ( $bpaf_activity_actions as $bpaf_key => $bpaf_label ) : ?>
							<?php if ( $bpaf_offer_as_default( $bpaf_key, $bpaf_default_filter ) ) : ?>
								<option value="<?php echo esc_attr( $bpaf_key ); ?>" <?php selected( $bpaf_default_filter, $bpaf_key ); ?>>
									<?php
									echo esc_html(
										in_array( $bpaf_key, $bpaf_hidden_activities, true )
											/* translators: %s: activity type label. */
											? sprintf( __( '%s (hidden)', 'bp-activity-filter' ), $bpaf_label )
											: $bpaf_label
									);
									?>
								</option>
							<?php endif; ?>
						<?php endforeach; ?>
					</select>
					<p class="description">
						<?php esc_html_e( 'Applied to the main site-wide activity stream the first time a visitor loads it.', 'bp-activity-filter' ); ?>
					</p>
					<?php if ( $bpaf_default_is_hidden ) : ?>
						<p class="description bpaf-description--warning">
							<?php esc_html_e( 'This type is hidden on the Hidden Activities tab, so the site-wide stream will show nothing. Pick a visible type, or make this one visible again.', 'bp-activity-filter' ); ?>
						</p>
					<?php endif; ?>
				</td>
			</tr>
			<tr>
				<th scope="row">
					<label for="bp_activity_filter_profile_default"><?php esc_html_e( 'Profile stream default', 'bp-activity-filter' ); ?></label>
				</th>
				<td>
					<select name="bp_activity_filter_profile_default" id="bp_activity_filter_profile_default" class="regular-text">
						<option value="-1" <?php selected( $bpaf_profile_default_filter, '-1' ); ?>>
							<?php esc_html_e( 'Everything', 'bp-activity-filter' ); ?>
						</option>
						<?php foreach ( $bpaf_activity_actions as $bpaf_key => $bpaf_label ) : ?>
							<?php
							if ( in_array( $bpaf_key, array( 'new_member', 'updated_profile' ), true ) ) {
								continue;
							}
							if ( ! $bpaf_offer_as_default( $bpaf_key, $bpaf_profile_default_filter ) ) {
								continue;
							}
							?>
							<option value="<?php echo esc_attr( $bpaf_key ); ?>" <?php selected( $bpaf_profile_default_filter, $bpaf_key ); ?>>
								<?php
								echo esc_html(
									in_array( $bpaf_key, $bpaf_hidden_activities, true )
										/* translators: %s: activity type label. */
										? sprintf( __( '%s (hidden)', 'bp-activity-filter' ), $bpaf_label )
										: $bpaf_label
								);
								?>
							</option>
						<?php endforeach; ?>
					</select>
					<p class="description">
						<?php esc_html_e( 'Applied to each member profile activity stream. Profile-irrelevant types such as "New Member" are left out of this list.', 'bp-activity-filter' ); ?>
					</p>
					<?php if ( $bpaf_profile_default_is_hidden ) : ?>
						<p class="description bpaf-description--warning">
							<?php esc_html_e( 'This type is hidden on the Hidden Activities tab, so member profile streams will show nothing. Pick a visible type, or make this one visible again.', 'bp-activity-filter' ); ?>
						</p>
					<?php endif; ?>
				</td>
			</tr>
		</tbody>
	</table>
</div>
