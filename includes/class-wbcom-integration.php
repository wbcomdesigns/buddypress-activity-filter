<?php // phpcs:ignore WordPress.Files.FileName.InvalidClassFileName -- Shared integration file.
/**
...
/**
 * Handles Wbcom Designs admin integration for the BP Activity Filter plugin.
 *
 * Registers the plugin with the shared admin system and provides the main
 * Wbcom Designs dashboard menu.
 *
 * @since 4.0.0
 */
class BP_Activity_Filter_Wbcom_Integration {
	...
	/**
	 * Whether the shared admin system loaded successfully.
	 *
	 * @var bool
	 */
	 ...
	 * Constructor.
	 *
	 * Sets up plugin data and initializes hooks.
	 *
	 * @since 4.0.0
	 */
	...
	private function is_wbcom_admin_page( $hook_suffix ) {
		...
		if ( in_array( $hook_suffix, $wbcom_pages, true ) ) {
			return true;
		}
		...
		if ( in_array( $page, $wbcom_page_slugs, true ) ) {
			return true;
		}
