...
class BP_Activity_Filter_Admin {
							...
							<?php else : ?>
								...
								<div>
									...
									<?php foreach ( $activity_actions as $key => $label ) : ?>
										...
										?>
										<div data-activity-state="<?php echo esc_attr( $is_checked ? 'hidden' : 'visible' ); ?>" style="display: block; margin-bottom: 8px; padding: 10px 12px; background: <?php echo esc_attr( $bg_color ); ?>; border: 1px solid <?php echo esc_attr( $border_color ); ?>; border-radius: 4px; transition: all 0.3s ease;">
											<label for="<?php echo esc_attr( $checkbox_id ); ?>" style="display: flex; align-items: center; cursor: pointer; margin: 0;">
												...
												<span class="dashicons <?php echo esc_attr( $icon ); ?>" style="color: <?php echo esc_attr( $icon_color ); ?>; margin-right: 8px;"></span>
												<span style="flex: 1; font-weight: 500; color: <?php echo esc_attr( $text_color ); ?>;"><?php echo esc_html( $label ); ?></span>
												<span style="font-size: 12px; font-weight: 600; color: <?php echo esc_attr( $icon_color ); ?>; margin-right: 10px;"><?php echo esc_html( $status_text ); ?></span>
				...
				<label style="display: flex; align-items: center; font-weight: 600;">
					...
					<span style="font-size: 12px; color: #666; margin-left: 8px;">
						...
						printf(
							...
							esc_html( _n( '%s post', '%s posts', $total_posts, 'bp-activity-filter' ) ),
	...
	private function save_settings() {
		$nonce = isset( $_POST['bp_activity_filter_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['bp_activity_filter_nonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'bp_activity_filter_save_settings' ) ) {
			add_settings_error(
	...
	private function save_default_filters() {
		...
		if ( isset( $_POST['bp_activity_filter_default'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$default_filter = $this->sanitize_default_filter( sanitize_text_field( wp_unslash( $_POST['bp_activity_filter_default'] ) ) );
		...
		if ( isset( $_POST['bp_activity_filter_profile_default'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$profile_default = $this->sanitize_default_filter( sanitize_text_field( wp_unslash( $_POST['bp_activity_filter_profile_default'] ) ) );
	...
	private function save_hidden_activities() {
		...
		if ( isset( $_POST['bp_activity_filter_hidden'] ) && is_array( $_POST['bp_activity_filter_hidden'] ) ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Missing
			$hidden = $this->sanitize_hidden_activities( array_map( 'sanitize_text_field', wp_unslash( $_POST['bp_activity_filter_hidden'] ) ) );
		}
		...
		<div class="bp-activity-filter-faq">
			...
			<div class="faq-item">
				...
				<p>
					<?php
					printf(
						/* translators: %1$s: opening link tag for support forum, %2$s: closing link tag, %3$s: opening link tag for documentation, %4$s: closing link tag. */
