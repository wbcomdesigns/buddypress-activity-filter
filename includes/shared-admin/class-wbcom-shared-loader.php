 ...
 *
 * Main coordinator for the Wbcom shared admin system.
 * Handles plugin registration, menu creation, and shared asset loading.
 */
class Wbcom_Shared_Loader {
	 ...
	 * Singleton instance.
	 *
	 * @var Wbcom_Shared_Loader|null
	 */
	private static $instance = null;

	/**
	 * Registered plugins data.
	 *
	 * @var array
	 */
	...
	/**
	 * Whether this is the primary loader instance.
	 *
	 * @var bool
	 */
	...
	/**
	 * Slug of the plugin that loaded this instance.
	 *
	 * @var string
	 */
	...
	/**
	 * Path to the shared admin directory.
	 *
	 * @var string
	 */
	private $shared_path = '';
	 ...
	 *
	 * @param string $slug The plugin slug.
	 * @return string|false The admin class name or false if not found.
	 */
	 ...
	 *
	 * @param string $slug The plugin slug.
	 * @return string The BuddyPress-style class name.
	 */
	 ...
	 * @param string $slug The plugin slug.
	 * @return string The dashicons class name.
	 */
	private static function pick_plugin_icon( $name, $slug ) {
		$name_lower = strtolower( $name . ' ' . $slug );

		if ( strpos( $name_lower, 'activity' ) !== false ) {
			return 'dashicons-admin-comments';
		}
		if ( strpos( $name_lower, 'member' ) !== false ) {
			return 'dashicons-admin-users';
		}
		if ( strpos( $name_lower, 'group' ) !== false ) {
			return 'dashicons-groups';
		}
		if ( strpos( $name_lower, 'message' ) !== false ) {
			return 'dashicons-email';
		}
		if ( strpos( $name_lower, 'notification' ) !== false ) {
			return 'dashicons-bell';
		}
		if ( strpos( $name_lower, 'profile' ) !== false ) {
			return 'dashicons-admin-users';
		}
		if ( strpos( $name_lower, 'media' ) !== false ) {
			return 'dashicons-admin-media';
		}
		if ( strpos( $name_lower, 'event' ) !== false ) {
			return 'dashicons-calendar';
		}
		if ( strpos( $name_lower, 'poll' ) !== false ) {
			return 'dashicons-chart-bar';
		}
		if ( strpos( $name_lower, 'quote' ) !== false ) {
			return 'dashicons-format-quote';
		}
		if ( strpos( $name_lower, 'hashtag' ) !== false ) {
			return 'dashicons-tag';
		}
		if ( strpos( $name_lower, 'filter' ) !== false ) {
			return 'dashicons-filter';
		}
		if ( strpos( $name_lower, 'social' ) !== false ) {
			return 'dashicons-share';
		}
		if ( strpos( $name_lower, 'buddypress' ) !== false ) {
			return 'dashicons-groups';
		}
	 ...
	 *
	 * @param array $plugin_data Plugin information.
	 * @return Wbcom_Shared_Loader The shared loader instance.
	 */
	 ...
	 *
	 * @param array $data Raw plugin data to validate.
	 * @return array|false Validated plugin data or false on failure.
	 */
	 ...
	 *
	 * @return string The detected shared path or empty string.
	 */
	 ...
	 *
	 * @param array $plugin_data Plugin information to register.
	 */
	...
	public function add_plugin_submenus() {
		foreach ( $this->registered_plugins as $plugin ) {
			if ( 'active' !== $plugin['status'] || empty( $plugin['settings_url'] ) ) {
				continue;
			}
	 ...
	 *
	 * @param string $admin_class The admin class name.
	 * @param array  $plugin      Plugin data array.
	 */
	...
	private function show_not_found_page() {
		...
		<div class="wrap">
			...
			<p><a href="<?php echo esc_url( admin_url( 'admin.php?page=wbcom-designs' ) ); ?>" class="button button-primary">&larr; Back to Dashboard</a></p>
		</div>
		<?php
	}
	 ...
	 *
	 * @param string $hook_suffix The current admin page hook suffix.
	 * @return bool Whether the current page is a Wbcom admin page.
	 */
	 ...
	 *
	 * @return string The URL to the shared admin assets directory.
	 */
	...
	/**
	 * Check if the Wbcom Designs menu already exists.
	 *
	...
	private function menu_exists() {
		...
		foreach ( $menu as $item ) {
			if ( isset( $item[2] ) && 'wbcom-designs' === $item[2] ) {
				return true;
			}
		}
	...
	/**
	 * Check if a submenu already exists under Wbcom Designs.
	 *
	 * @param string $menu_slug The submenu slug to check.
	 * @return bool Whether the submenu exists.
	 */
	...
	/**
	 * Extract the menu slug from a settings URL.
	 *
	 * @param string $settings_url The settings URL to parse.
	 * @return string The extracted menu slug.
	 */
	...
	/**
	 * Get the SVG menu icon for Wbcom Designs.
	 *
	 ...
	 *
	 * @return array Registered plugins data.
	 */
	...
	/**
	 * Get only active registered plugins.
	 *
	...
	/**
	 * Get the global shared loader instance.
	 *
	 * @return Wbcom_Shared_Loader|null The instance or null.
	 */
	...
	/**
	 * Get debug information about the shared system.
	 *
